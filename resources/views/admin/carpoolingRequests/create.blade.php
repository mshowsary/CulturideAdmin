...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.carpoolingRequest.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.carpooling-requests.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <div class="form-check {{ $errors->has('accepted') ? 'is-invalid' : '' }}">
                    <input class="form-check-input" type="checkbox" name="accepted" id="accepted" value="1" required {{ old('accepted', 0) == 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('carpooling') ? 'is-invalid' : '' }}" name="carpooling_id" id="carpooling_id" required>
                    @foreach($carpoolings as $id => $entry)
                        <option value="{{ $id }}" {{ old('carpooling_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
