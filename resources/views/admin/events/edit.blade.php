...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.events.update", [$event->id]) }}" enctype="multipart/form-data">
            ...
            @can('role_create')
            <div class="form-group">
                <div class="form-check {{ $errors->has('home') ? 'is-invalid' : '' }}">
                    <input type="hidden" name="home" value="0">
                    <input class="form-check-input" type="checkbox" name="home" id="home" value="1" {{ $event->home || old('home', 0) === 1 ? 'checked' : '' }}>
                    <label class="form-check-label" for="home">{{ trans('cruds.event.fields.home') }}</label>
                </div>
                @if($errors->has('home'))
                    <div class="invalid-feedback">
                        {{ $errors->first('home') }}
                ...
                @endif
                <span class="help-block">{{ trans('cruds.event.fields.home_helper') }}</span>
            </div>
            @endcan
            <div class="form-group">
                <label class="required" for="date">{{ trans('cruds.event.fields.date') }}</label>
                <input class="form-control datetime {{ $errors->has('date') ? 'is-invalid' : '' }}" type="text" name="date" id="date" value="{{ old('date', $event->date) }}" required>
                @if($errors->has('date'))
                    <div class="invalid-feedback">
                        {{ $errors->first('date') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('time') ? 'is-invalid' : '' }}" type="text" name="time" id="time" value="{{ old('time', $event->time) }}" required>
                @if($errors->has('time'))
                    <div class="invalid-feedback">
                        {{ $errors->first('time') }}
                    </div>
            ...
            <div class="form-group">
                <label class="required" for="artist_id">{{ trans('cruds.event.fields.artist') }}</label>
                <select class="form-control select2 {{ $errors->has('artist') ? 'is-invalid' : '' }}" name="artist_id" id="artist_id" required>
                    @foreach($artists as $id => $entry)
                        <option value="{{ $id }}" {{ (old('artist_id') ? old('artist_id') : $event->artist->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.event.fields.artist_helper') }}</span>
            </div>
            <div class="form-group">
                <label class="required" for="category_id">{{ trans('cruds.event.fields.category') }}</label>
                <select class="form-control select2 {{ $errors->has('category') ? 'is-invalid' : '' }}" name="category_id" id="category_id" required>
                    @foreach($categories as $id => $entry)
                        <option value="{{ $id }}" {{ (old('category_id') ? old('category_id') : $event->category->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <label class="required" for="type_id">{{ trans('cruds.event.fields.type') }}</label>
                <select class="form-control select2 {{ $errors->has('type') ? 'is-invalid' : '' }}" name="type_id" id="type_id" required>
                    @foreach($types as $id => $entry)
                        <option value="{{ $id }}" {{ (old('type_id') ? old('type_id') : $event->type->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('type'))
                    <div class="invalid-feedback">
