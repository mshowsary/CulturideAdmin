    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.tickets.update", [$ticket->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('seat') ? 'is-invalid' : '' }}" type="number" name="seat" id="seat" value="{{ old('seat', $ticket->seat) }}" step="1" required>
                @if($errors->has('seat'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('codebar') ? 'is-invalid' : '' }}" type="text" name="codebar" id="codebar" value="{{ old('codebar', $ticket->codebar) }}" required>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('used') ? 'is-invalid' : '' }}">
                    <input class="form-check-input" type="checkbox" name="used" id="used" value="1" {{ $ticket->used || old('used', 0) === 1 ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('zone') ? 'is-invalid' : '' }}" name="zone_id" id="zone_id" required>
                    @foreach($zones as $id => $entry)
                        <option value="{{ $id }}" {{ (old('zone_id') ? old('zone_id') : $ticket->zone->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('zone'))
                    <div class="invalid-feedback">
